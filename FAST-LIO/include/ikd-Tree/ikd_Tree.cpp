...
{
    // 设定平衡判断参数
    delete_criterion_param = delete_param; // alpha-deleted 预设0.5
    balance_criterion_param = balance_param; // alpha-balanced 预设0.7
    // I(T) < alpha-deleted S(T)
    // S(T,leftson) < alpha-bal (S(T)-1) and S(T,rightson) < alpha-bal (S(T)-1)
    // 满足上述条件，该子树平衡
...
void KD_TREE::InitTreeNode(KD_TREE_NODE *root)
{
    ...
    // pushdown是用来从根节点向下同步信息的，例如tree_deleted
...
int KD_TREE::size()
{
    ...
    else
    {
        ...
        if (!pthread_mutex_trylock(&working_flag_mutex)) // 成功。pthread_mutex_trylock在成功时返回0
        {
            // 返回根节点汇总得到的有效节点个数
...
{
    BoxPointType range;
    // 没有重建树，或没有重建整棵树，根节点的node_range就没有其他线程在访问
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node)
    {
        if (Root_Node != nullptr)
        {
...
// 并且加锁和解锁还可以使用lock_guard<std::mutex>自动解锁
void KD_TREE::start_thread()
{
    ...
    // 第二个参数用来设置线程属性
    // 第三个参数是线程运行函数的起始地址
    // 最后一个参数是运行函数的参数 因为参数类型是void* 需要强制转型
...
// 此函数返回值必须是void* 否则pthread_create会报错
...
int KD_TREE::Add_Points(PointVector &PointToAdd, bool downsample_on)
{
    ...
    for (int i = 0; i < PointToAdd.size(); i++)
    {
        ...
        if (downsample_switch)
        {
            ...
            { // 需要重建树 如果有re-balancing任务在并行运行，在对当前tree执行`BoxDelete`和`插入一个点`之外，还需要把这些操作缓存到logger里。
                if (Downsample_Storage.size() > 1 || same_point(PointToAdd[i], downsample_result))
                {
        ...
        { // 不需要降采样
            ...
            { // 如果有并行re-balancing任务，还需额外把当前操作放入logger缓存，等重建完成后补作业
                Operation_Logger_Type operation;
...
void KD_TREE::BuildTree(KD_TREE_NODE **root, int l, int r, PointVector &Storage)
{
    ...
    ******************************************************/
    ...
    // 更新node的属性 放在这个位置，迭代的建立了所有的子树，会从叶子节点自下而上的update上层的节点
    Update((*root));
    return;
}
...
void KD_TREE::Rebuild(KD_TREE_NODE **root)
{
    ...
    // 树的节点数大于阈值，即使用另一线程进行树的重建操作，这一操作是在另一线程一直运行的，只要Rebuild_Ptr不为null，即开始重建
...
void KD_TREE::Delete_by_point(KD_TREE_NODE **root, PointType point, bool allow_rebuild)
{
    ...
    {
        (*root)->point_deleted = true;   // 打上'删除标签'，但并不是直接释放内存，而是等下次重建时释放，重建是在flatten函数中释放的
...
void KD_TREE::Add_by_point(KD_TREE_NODE **root, PointType point, bool allow_rebuild, int father_axis)
{
    ...
    if (((*root)->division_axis == 0 && point.x < (*root)->point.x) || ((*root)->division_axis == 1 && point.y < (*root)->point.y) || ((*root)->division_axis == 2 && point.z < (*root)->point.z))
    {
        ...
        { // 该左子树正在重建的话，则将该操作记录到Rebuild_Logger，待重建完成后补作业
            // 线程互斥锁
            pthread_mutex_lock(&working_flag_mutex);
...
bool KD_TREE::Criterion_Check(KD_TREE_NODE *root)
{
    ...
    // 代码在update函数中也做了判断，但是是把balance_evaluation确定为了左右子树中较大的那一个，因此只比较了大于param的那部分
...
void KD_TREE::Push_Down(KD_TREE_NODE *root)
{
    ...
    if (root->need_push_down_to_left && root->left_son_ptr != nullptr)
    {
        ...
        { // if条件的逆否命题：当前需要重建 并且 需要重建的树是左子树
            pthread_mutex_lock(&working_flag_mutex);
            // 将当前节点的删除标签传递给左子节点
            root->left_son_ptr->tree_downsample_deleted |= root->tree_downsample_deleted;
            root->left_son_ptr->point_downsample_deleted |= root->tree_downsample_deleted;
...
{ // 该函数不需要递归，因为父节点的属性只与左右子节点相关
    ...
    if (root == Root_Node && root->TreeSize > 3)
    {
        ...
        if (son_ptr == nullptr)
            son_ptr = root->right_son_ptr; // 左子树为null则获取右子树
        ...
        // 这里计算左子树的节点数占总节点数的比例，如果大于一半，即赋值给alpha_bal；否则，选取右子树的比例赋值
        // 也就是选取子树中含有较多节点的一方，作为判断平衡树的条件
...
void MANUAL_HEAP::MoveDown(int heap_index)
{
    ...
    while (l < heap_size)
    {
        ...
        if (tmp < heap[l])
        {
            ...
            // 这里只把子节点中大的放在了父节点中，然后把当前heap_index的位置留成了空穴
            heap_index = l;
            l = heap_index * 2 + 1;
        }
        else
...
// 最大堆的两条策略：
// 1. push新的节点（自下向上）。在数组末尾创建一个空穴（先不赋值），逐次用新加入节点的数值跟父节点比较，如果父节点小，就把父节点移到空穴。
// 最后，直到父节点的值大于新节点的值，停止上滤，给空穴赋值。
// 2. pop出堆顶的节点（自上向下）。把堆中末尾的最后一个节点送入堆顶，维护其完全二叉树的结构，但实际是把末尾节点数值暂存，把堆顶设为空穴。
// 逐次用左右子节点中较大的数值跟存下的数值比较，如果子节点较大，则交换位置。直到子节点小于存下的数值，把此位置赋值为存下的（数组末尾节点的）数值。
